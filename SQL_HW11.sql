...
END AS rate_category
 ...
 -- 1. подзапрос в SELECT c exists с корелляцией по id;
...
-- 2. подзапрос в SELECT для вывода имени и фамилии каждого оператора 
...
-- 3. подзапроc в SELECT вывести для каждого города его название
...
-- 4. Подзапрос в SELECT вывеcти имена и фамилии клиентов
...
-- 7. подзапрос с UPDATE по обновлению мест, чтобы столбец available_seats отражал количество свободных мест;
update trip as t
 join		   (
				select trip_id, 
